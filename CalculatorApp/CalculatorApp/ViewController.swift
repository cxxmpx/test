...
//  CalculatorApp
//
//  Created by HiroshiYamaguchi on 2018/10/29.
//  Copyright © 2018 HiroshiYamaguchi. All rights reserved.
//
...
class ViewController: UIViewController {
    @IBOutlet weak var formulLabel: UILabel!
    ...
    @IBAction func inputFormula(_ sender: UIButton) {
        ...
        guard let formulaText = formulLabel.text else {
            return
        }
        guard let senderedText = sender.titleLabel?.text else {
            return
        }
        formulLabel.text = formulaText + senderedText
    }
